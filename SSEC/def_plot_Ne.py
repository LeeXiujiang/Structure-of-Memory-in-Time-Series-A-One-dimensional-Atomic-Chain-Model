...
def generate_gradient_seaborn(color_list, N=256):
    ...
    return palette
def plot_Ne(plot_eig,Ne,alpha_1_list,color_gradient=True,plot_one = True): 
    # if color_gradient:
    #     cmap = plt.cm.get_cmap('viridis')
        # # 定义颜色渐变范围（从蓝色到红色）
    len_H = len(alpha_1_list)
    colors = plt.cm.get_cmap('coolwarm', (len_H-1))  # 'coolwarm' 是 matplotlib 内置的渐变色
    # # 生成 20 个渐变色
    ...
    if plot_one:
        ...
        for i,alpha_1 in enumerate(alpha_1_list):
            ...
            plt.plot(plot_eig[f"alpha_1={alpha_1:.2f}"],Ne[f"alpha_1={alpha_1:.2f}"],c=color)
            plt.scatter(plot_eig[f"alpha_1={alpha_1:.2f}"],Ne[f"alpha_1={alpha_1:.2f}"],c = color,label = fr"$\alpha_1$ = {alpha_1:.1f}")
            plt.xlabel(r"$E$")
            plt.ylabel(r"$\rho(\bar{E})$")

            # plt.loglog(plot_eig[f"H={h:.2f}"],Ne[f"H={h:.2f}"],"o",label=h)
            plt.legend()
         # 调整边距，确保标题和标签不被遮挡
        plt.subplots_adjust(left=0.1, right=0.9, top=0.95, bottom=0.12)  # 调整四个方向的边距
        plt.show()   
    else:
        ...
        for i,alpha_1 in enumerate(alpha_1_list):
            plt.subplot(m,n,i+1)
            plt.title(fr"$\alpha_1$ = {alpha_1:.1f}")
            plt.plot(plot_eig[f"alpha_1={alpha_1:.2f}"],Ne[f"alpha_1={alpha_1:.2f}"],color_list[i])
            plt.scatter(plot_eig[f"alpha_1={alpha_1:.2f}"],Ne[f"alpha_1={alpha_1:.2f}"],c= color_list[i])
            plt.legend()
        # plt.legend()
        plt.show()
