...
def getunfoled_form_matrix(H):
    ...
    for i,h in enumerate(H):
        ...
        file_name = f"Data\\toe_xieduichen_{h:.2f}_2000_100_p.csv"
...
def getunfoled_form_matrix_quanpu(H):
    ...
    for i,h in enumerate(H):
        print(f"processing H = {h:.2f}")
        file_name = f"Data\\toe_{h:.2f}_2000_100_p.csv"
        df = pd.read_csv(file_name, header=None)
        matrix = df.values
...
def getunfoled_form_matrix_duichenzipu(H):
    ...
    for i,h in enumerate(H):
        print(f"processing H = {h:.2f}")
        file_name = f"Data\\toe_duichen_{h:.2f}_2000_100_p_2.csv"
        df = pd.read_csv(file_name, header=None)
        matrix = df.values
...
def getunfoled_form_matrix_xieduichenzipu(H):
    ...
    for i,h in enumerate(H):
        print(f"processing H = {h:.2f}")
        file_name = f"Data\\toe_xieduichen_{h:.2f}_2000_100_p_2.csv"
        df = pd.read_csv(file_name, header=None)
        matrix = df.values
...
L = np.arange(1,30,0.2)
all_means = {"full":{}, "sym":{}, "skew":{}}
color_a = "#2A60D9"
color_b = "#559BD4"
color_c = "#73C2CD"
# color_list = ["#379863","#F7922C","#8762A8","#63A7DC"]
...
base_dir="delta\\"
H = [0.60,0.70,0.80,0.90]
H_title = [0.60,0.70,0.80,0.90]
for idx, h in enumerate(H_title):
    ...
    if idx == 0:
        ax.plot(L, p, color="green", linestyle="-", linewidth=2.5/scale_change, label="Poisson")
        ax.plot(L, g, color="orange", linestyle="-", linewidth=2.5/scale_change, label="GOE")
    ax.scatter(L, m_full, color=color_list[idx], marker="D",s=30/scale_change_2,linewidths=1.5/scale_change)
...
plt.savefig("fig05.png", dpi=600, bbox_inches='tight',pad_inches=0.05)
plt.savefig("fig05.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
plt.savefig("fig05.svg", dpi=600, bbox_inches='tight', pad_inches=0.1)
plt.savefig("fig05.eps", dpi=600,format= 'eps', bbox_inches='tight',pad_inches=0.05)
plt.show()
