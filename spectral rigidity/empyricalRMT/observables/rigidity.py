...
#
# 定义：
# ∆3(L)表示在能量尺度L范围内，展开后的累积能级密度N(E)与最佳拟合直线
# 之间的最小均方差，是表征能级关联特性的重要指标
#
# 物理意义：
# - 对于泊松分布（可积系统）：∆3(L) ~ L/15
# - 对于GOE分布（量子混沌系统）：∆3(L) ~ (ln L)/π² + const
# - 对于GUE分布：∆3(L) ~ (ln L)/2π² + const
# - 对于GSE分布：∆3(L) ~ (ln L)/4π² + const
#
# 计算步骤：
# 1. 对原始能谱{E_i}进行能级展开，得到展开能级{ε_i}
# 2. 在展开能级区间内随机选取起始点c，区间长度L∈[L_min, L_max]
# 3. 统计落在[c,c+L]区间内的能级数n，要求n ≥ 3
# 4. 对区间内累积能级数N(ε)进行线性拟合N_fit(ε) = aε + b
# 5. 计算方差积分：
#    ∆3(c,L) = (1/L) ∫[c,c+L] [N(ε) - N_fit(ε)]² dε
# 6. 重复多次取平均得到∆3(L)统计量
#
# 注意事项：
# - 典型参数选择：L_min=2，L_max=15（以平均能级间距为单位）
# - 需要足够多的采样次数以保证统计可靠性
# - 能级展开步骤对最终结果影响显著
