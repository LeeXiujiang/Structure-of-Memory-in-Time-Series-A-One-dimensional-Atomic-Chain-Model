...
def _spectral_rigidity(
    ...
    s: float = 30,
    linewidth: float = 1.5,
    label_add: str = "spectral rigidity",
    **kwargs: Mapping,
) -> PlotResult:
    ...
    sbn.scatterplot(
        ...
        marker=marker,
        s = 40/scale_change_2,
        linewidth = 1/scale_change,
        # linewidth=linewidth/ scale_change,  # 控制边框宽度
        facecolor="none" if (kongxin or label_add == "Skew-symmetric") else color_a,
        label = None if (kongxin or label_add == "No label") else label_add
        # s=s,  # 控制点的大小
        # linewidth=linewidth  # 控制边框宽度
    )
    # axes.scatter(
    #     df_converged["L"],
    #     df_converged["delta"],
    ...
    #     edgecolors=color_a,
    #     facecolors="none" if (kongxin or label_add == "Skew-symmetric") else color_a,

    #     linewidths=linewidth/scale_change,   # 控制线框宽度（复数形式）
    #     # if label_add = ""
    #     # 逻辑：如果 (kongxin=False) OR (label_add != "No label")，显示 label_add，否则显示 None
    #     # label = label_add if (not kongxin or label_add != "No label") else None
    #     label = None if (kongxin or label_add == "No label") else label_add
    #     # label = label_add if not kongxin else None,
    #     # label = None if label_add == "No label" else label_add
    # )
    ...
    #     edgecolors=color_a,        # 用复数
    #     facecolors="none" if (kongxin or label_add == "Skew-symmetric") else color_a,

    #     linewidths=linewidth,      # 用复数
    # )
    ...
    if "goe" in ensembles:
        ...
        goe = axes.plot(L, goe, label="GOE", color="#FD8208")
        # plt.setp(goe, color="#FD8208")
    if "gue" in ensembles:
        gue = (1 / (2 * (p**2))) * (np.log(2 * p * s) + y - 5 / 4)
        gue = axes.plot(L, gue, label="GUE", color="#0066FF")
        # plt.setp(gue, color="#0066FF")
    if "gse" in ensembles:
        gse = (1 / (4 * (p**2))) * (np.log(4 * p * s) + y - 5 / 4 + (p**2) / 8)
        gse = axes.plot(L, gse, label="GSE ", color="#EA00FF")
        # plt.setp(gse, color="#EA00FF")
