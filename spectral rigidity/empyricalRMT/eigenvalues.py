...
class Eigenvalues(EigVals):
    ...
    @staticmethod
    def get_eig_from_mat(matrix,percent=True,lower = 0.2,upper = 0.8) -> Eigenvalues:
        eig = matrix.flatten()
        if percent:
            eig = eig[int(len(eig)*lower):int(len(eig)*upper)]
        return Eigenvalues(np.sort(eig))
    @staticmethod
    def get_eig_from_list(list,percent=True,lower = 0.2,upper = 0.8) -> Eigenvalues:
        if percent:
            list = list[int(len(list)*lower):int(len(list)*upper)]
        return Eigenvalues(np.sort(list))
    @staticmethod
    def generate(
        matsize: int,
    ...
    def unfold(
        ...
        smoother: "poly" | "spline" | "gompertz" | "goe" | lambda | None
            ...
            - "poly"：多项式展开
            - "spline"：使用单变量样条拟合
            - "gompertz"：拟合Gompertz指数曲线
            - "goe"：通过半圆律进行"平滑"展开
            - lambda：未实现
            - None (默认值)：
            - 如果通过Eigenvalues.generate(..., kind="goe")构造，
            则使用解析展开方法，即smoother="goe"
            - 如果通过Eigenvalues.generate(..., kind="poisson")构造，
            则使用19次多项式
        ...
        spline_smooth: float
            传递给scipy.interpolate.UnivariateSpline的平滑因子

        emd_detrend: bool
            是否在返回最终展开值前应用经验模态分解去趋势（Morales等人方法）
